    ...
    fn simplify(&self, token_stream: &mut TokenStream) -> Result<(), DiagnosticBuilder> {
        fn apply_results(
            ...
            if num_part.as_f64() != 1.0 {
                ...
                let sign = if num_part.is_negative() {
                    SignKind::Minus
                } else {
                    SignKind::Default
                };
                let mut buff = num_part.to_string();
    ...
    fn simplify(&self, token_stream: &mut TokenStream) -> Result<(), DiagnosticBuilder> {
        fn apply_results(
            ...
            if !num_part.is_zero() {
                ...
                let negative = num_part.is_negative();
                let mut buff = num_part.to_string();
                if negative {
                    buff.remove(0);
                }
...
fn test() {
    ...
    _lib::simplify(String::from("8*4+6*0+4*3*5*0+3*0+0*3*9"), &mut context).unwrap();
    assert_eq!(context.parse_ctx.read().unwrap().get_input(), "32");
    _lib::simplify(String::from("0+0*(8+3)-(8+3)*0"), &mut context).unwrap();
    assert_eq!(context.parse_ctx.read().unwrap().get_input(), "0");
    _lib::simplify(String::from("x*0"), &mut context).unwrap();
    assert_eq!(context.parse_ctx.read().unwrap().get_input(), "0");
    _lib::simplify(String::from("8*4+4*3*5*0+x"), &mut context).unwrap();
    assert_eq!(context.parse_ctx.read().unwrap().get_input(), "32+x");
    _lib::simplify(String::from("1/4*8*x"), &mut context).unwrap();
    assert_eq!(context.parse_ctx.read().unwrap().get_input(), "2*x");
    _lib::simplify(String::from("y*(4)+2"), &mut context).unwrap();
    assert_eq!(context.parse_ctx.read().unwrap().get_input(), "y*4+2");
    _lib::simplify(String::from("0/(5*3+4)"), &mut context).unwrap();
    assert_eq!(context.parse_ctx.read().unwrap().get_input(), "0");
    _lib::simplify(String::from("((5*3+x))"), &mut context).unwrap();
